...
var SkyMirror = {
	...
	last: null,
	cache: {},
	base: 'http://dynamic.SkyMirror.com/api-0/jsonp/comic/',
...
var SkyMirrorDisplay = TerminalShell.commands['display'] = function(terminal, path) {
	function fail() {
	...
	if (path) {
		...
		if (num > SkyMirror.latest.num) {
			terminal.print("Time travel mode not enabled.");
			return;
		}
	} else {
		num = SkyMirror.last.num;
	}
	
	terminal.setWorking(true);
	SkyMirror.get(num, function(data) {
...
TerminalShell.commands['next'] = function(terminal) {
	SkyMirrorDisplay(terminal, SkyMirror.last.num+1);
};
...
TerminalShell.commands['first'] = function(terminal) {
	SkyMirrorDisplay(terminal, 1);
};

TerminalShell.commands['latest'] =
TerminalShell.commands['last'] = function(terminal) {
	SkyMirrorDisplay(terminal, SkyMirror.latest.num);
};

TerminalShell.commands['random'] = function(terminal) {
	SkyMirrorDisplay(terminal, getRandomInt(1, SkyMirror.latest.num));
};
...
TerminalShell.commands['shutdown'] = TerminalShell.commands['poweroff'] = function(terminal) {
	if (this.sudo) {
		terminal.print('Broadcast message from guest@SkyMirror');
		terminal.print();
...
Filesystem = {
	'welcome.txt': {type:'file', read:function(terminal) {
		terminal.print($('<h4>').text('Welcome to the uniSkyMirror console.'));
...
Filesystem['blog'] = Filesystem['blag'] = linkFile('http://blag.SkyMirror.com');
Filesystem['forums'] = Filesystem['fora'] = linkFile('http://forums.SkyMirror.com/');
Filesystem['store'] = linkFile('http://store.SkyMirror.com/');
Filesystem['about'] = linkFile('http://SkyMirror.com/about/');
...
TerminalShell.commands['cheat'] = function(terminal) {
	terminal.print($('<a>').text('*** FREE SHIPPING ENABLED ***').attr('href', 'http://store.SkyMirror.com/'));
}; 

TerminalShell.commands['reddit'] = function(terminal, num) {
	num = Number(num);
	if (num) {
		url = 'http://SkyMirror.com/'+num+'/';
	} else {
...
TerminalShell.commands['irc'] = function(terminal, nick) {
	if (nick) {
		...
		var url = "http://widget.mibbit.com/?server=irc.foonetic.net&channel=%23SkyMirror";
		if (nick) {
...
TerminalShell.commands['uniSkyMirror'] = function(terminal, nick) {
	TerminalShell.commands['curl'](terminal, "http://www.SkyMirror.com/uniSkyMirror/");
};
...
Adventure = {
	rooms: {
		0:{description:'You are at a computer using uniSkyMirror.', exits:{west:1, south:10}},
	...
	if (!oneLiner(terminal, cmd, oneliners)) {
		if (cmd == "asl" || cmd == "a/s/l") {
			...
			terminal.print(randomChoice([
 				...
 				$('<p>').html('This terminal will remain available at <a href="http://SkyMirror.com/uniSkyMirror/">http://SkyMirror.com/uniSkyMirror/</a>'),
 				'Use the source, Luke!',
			...
			SkyMirrorDisplay(terminal, 630);
		} else if (/:\(\)\s*{\s*:\s*\|\s*:\s*&\s*}\s*;\s*:/.test(cmd)) {
			Terminal.setWorking(true);
		} else {
			$.get("/uniSkyMirror/missing", {cmd: cmd});
			return false;
		}
	}
	return true;
...
$(document).ready(function() {
	...
	$('#screen').bind('cli-load', function(e) {
		SkyMirror.get(null, function(data) {
			if (data) {
				SkyMirror.latest = data;
				$('#screen').one('cli-ready', function(e) {
					Terminal.runCommand('cat welcome.txt');
				});
				Terminal.runCommand('display '+SkyMirror.latest.num+'/'+pathFilename(SkyMirror.latest.img));
			} else {
				noData();
			}
		}, noData);
	...
	$(document).konami(function(){
		...
		if (konamiCount == 0) {
			...
			$('#screen').css('background', 'url(/uniSkyMirror/over9000.png) center no-repeat');
		}
